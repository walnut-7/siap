marss.wrapper = function(data, job, instance, model, p, r, diff = F,...) {
  ...
  S.test.w <- setdiff(which(apply(x_big, 2, function(v) all(is.na(v)))), which(apply(data, 2, function(v) all(is.na(v)))))
  ...
  list(x = x, x_pred=x_pred, x_sd=x_sd, # x_boots = x_boots,
       ...
       #ytT=fit$ytT, bootsamples=bootsamples, 
       #instance = instance
       )
}
