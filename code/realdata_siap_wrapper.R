siap.wrapper <- function(data, job, instance, p.cal, cp.alpha, ...) { 
  ...
  ttl_coverage.w <- sum((abs(x_imp - data)<=x_q)*Ome.test.w, na.rm = T)
  ttl_coverage.o <- sum((abs(x_imp - data)<=x_q)*Ome.test.o, na.rm = T)
  ave_coverage_rate <- (ttl_coverage.w + ttl_coverage.o)/sum(Ome.test)
  ...
  list(x = x, rae = list(rae.w=rae.w, rae.o=rae.o), S.test.w = S.test.w, S.test.o = S.test.o,
